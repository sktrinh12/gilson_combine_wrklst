...
def grab_sam_plt_nums(file1):
    for i in reversed(range(len(file1))):
        if not re.search('\t0.?\n',file1[i]): # if do not see the 0 in the sample well pos (it's a sample not std/flush)
            str_start_sample_pos = re.search('\t(?![0])\d{1,2}\t', file1[i]).group(0)
            str_start_plt_loc = re.search('\tP\d{1,2}S', file1[i]).group(0)
            break
    int_start_sample_pos = int(str_start_sample_pos)
    int_start_plt_loc = int(str_start_plt_loc[2:len(str_start_plt_loc)-1]) #rid of 0
...
if __name__ == "__main__":
    if len(sys.argv[1]) == 10 and len(sys.argv[2]) == 10:
        ...
        if check_file_content(file2) and check_file_content(file1):
            int_start_sample_num, int_start_plt_loc = grab_sam_plt_nums(file1) # grab the sample pos and plate loc
