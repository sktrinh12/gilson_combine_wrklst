...
if __name__ == "__main__":
    ...
    if check_reg_path_exists(reg_path):
        file_path = query_registry(reg_path, reg_key_val)
    else:
        file_path = 'blank/no/file_path/found' 
        print('registry path does not seem to exist')
        
    if file_path.endswith('.tsl') and \
            split_string_input_file(file_path).startswith('15') and \
            os.path.exists(file_path):
        
            imbue_rows(file_path)
            print('Successfully uploaded worklist to Oracle database')
    else:
        print(f'not a valid file type or file cannot be found! - {file_path}')